...
type Section = "review" | "runs" | "duplicates" | "sources";
...
export function AdminDashboard({ initialTasks, overview }: { initialTasks: ReviewTask[]; overview: AdminOverview }) {
  const [section, setSection] = useState<Section>("review");
  const labels = { review: ["Revisión editorial", "Candidatos pendientes de validar antes de afectar el mapa público."], runs: ["Ejecuciones de ingesta", "Historial de procesos ejecutados por cada adaptador."], duplicates: ["Posibles duplicados", "Fuentes distintas con el mismo contenido capturado."], sources: ["Fuentes registradas", "Documentos oficiales y sus últimas capturas disponibles."] } as const;
